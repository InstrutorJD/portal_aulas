...
// prática), o módulo "Prova Diagnóstica — Turma Jogos Digitais" e o módulo
// "Prova Final — Turma Jogos Digitais" — usados como base do % geral. O %
// de cada aluno é a MÉDIA da fração current/total de cada um dos 67
...
const SEED = {
  ...
  student_module_progress: [
    ...
    // js/intermediario) → soma 2 frações de 1.0 / 67 módulos = 2,99% → arredonda 3%.
...
test.describe('Ranking do aluno na turma', () => {
  ...
  test('professor gera o Ranking da Turma por botão, ordenado, com concluídas/disponíveis', async ({ page }) => {
    ...
    await expect(linhas.nth(0)).toContainText('2/67');
    ...
    await expect(linhas.nth(1)).toContainText('0/67');
  });
