// @ts-check
// "Prova Final — Turma Sistemas" (matéria Prova, trilha Prova Final):
// atividade SEPARADA da Prova Diagnóstica (prova-sistemas.spec.js) — banco
...
const PROVA_URL = '/turmas/sistemas/atividades/prova-final-sistemas.html?user=alexandre.natal&role=aluno&turma=sistemas';
...
test.describe('turmas/sistemas/atividades/prova-final-sistemas.html', () => {
  ...
  test('mostra a tela de regras antes de iniciar, mencionando teoria e prática', async ({ page }) => {
    ...
    await expect(page.locator('#gateWrap')).toContainText('Prova Final — Turma Sistemas');
  ...
  test('responde as 25 questões e conclui, sem opção de tentar de novo', async ({ page }) => {
    ...
    const stored = await page.evaluate(u => JSON.parse(localStorage.getItem(`prova_final_sistemas_progress_${u}`)), 'alexandre.natal');
  ...
  test('1ª saída da aba avisa, 2ª bloqueia — e o professor desbloqueia com o token RETOMANDO a mesma tentativa', async ({ page }) => {
    ...
    await answerCurrentQuestionSomehow(page);
    const selectionBefore = await page.evaluate(u => localStorage.getItem(`prova_final_sistemas_selecao_${u}`), 'alexandre.natal');
    ...
    const selectionAfter = await page.evaluate(u => localStorage.getItem(`prova_final_sistemas_selecao_${u}`), 'alexandre.natal');
  ...
  test('professor tem um botão "Reiniciar prova" pra testar o sorteio de novo', async ({ page }) => {
    const PROF_URL = '/turmas/sistemas/atividades/prova-final-sistemas.html?user=admin&role=professor&turma=sistemas';
    ...
    const selectionBefore = await page.evaluate(u => localStorage.getItem(`prova_final_sistemas_selecao_${u}`), 'admin');
    expect(selectionBefore).not.toBeNull();

    await page.click('#btnResetProfessor');

    const selectionAfter = await page.evaluate(u => localStorage.getItem(`prova_final_sistemas_selecao_${u}`), 'admin');
  ...
  test('gabarito lista o banco inteiro de 80 questões', async ({ page }) => {
    await page.goto('/turmas/sistemas/atividades/prova-final-sistemas.html?user=admin&role=professor&turma=sistemas');

    const download = await gerarGabaritoFlutuante(page);
    expect(download.suggestedFilename()).toBe('prova-final-sistemas-gabarito.txt');
...
test.describe('turmas/sistemas/plataforma.html — matéria Prova', () => {
  test('aparece a trilha Prova Final, separada da Prova Diagnóstica', async ({ page }) => {
    ...
    await expect(page.locator('#moduleSelector_prova-final')).toContainText('Prova Final — Turma Sistemas');
  });
});
